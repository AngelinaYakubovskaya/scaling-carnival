...
void turnRight(char* inputBuffer, char* outputBuffer, int width, int height, int sizeOfPixel)
{
...
void turnLeft(char* inputBuffer, char* outputBuffer, int width, int height, int sizeOfPixel)
{
...
void applyGaussianFilter(char* inputBuffer, char* outputBuffer, int width, int height, int sizeOfPixel) {
...
void writeBuffer(const char* file, ifstream& is, BITMAPINFOHEADER bmpInfoHeader, char* buffer, int bufferSize,
    int sizeOfPixel, int shiftToData, bool isHeadChanged = true)
{
    ...
    if (isHeadChanged)
    {
        ...
        os.write(reinterpret_cast<char*>(&bmpInfoHeader), sizeof(BITMAPINFOHEADER));
    }

    os.seekp(shiftToData); // Перемещаем указатель к началу пиксельных данных
    os.write(buffer, bufferSize * sizeOfPixel); // Записывает данные размером newBufferSize байт из буфера newBuffer в поток os
    os.close();
...
int main()
{
    ...
    if (!is)
    {
        cerr << "File can't be opened!" << endl; // Выводит сообщение об ошибке в стандартный поток ошибок
        return 1;
    }
    ...
    writeBuffer("Gauss.bmp", is, bmpInfoHeader, bufferGauss, bufferSize, sizeOfPixel, shiftToData, false);

    // write to right file -----------------------------
    char* rightTransformBuffer = new char[sizeOfPixel * bufferSize];
    turnRight(buffer, rightTransformBuffer, width, height, sizeOfPixel);
    writeBuffer("Rotated1.bmp", is, bmpInfoHeader, rightTransformBuffer, bufferSize, sizeOfPixel, shiftToData);

    // write to left file -----------------------------
    char* leftTransformBuffer = new char[sizeOfPixel * bufferSize];
    turnLeft(buffer, leftTransformBuffer, width, height, sizeOfPixel);
    writeBuffer("Rotated2.bmp", is, bmpInfoHeader, leftTransformBuffer, bufferSize, sizeOfPixel, shiftToData);
